...
root.resizable(width=True, height=True)  # The window size of the game.
root.geometry('1000x750')
root.configure(background='green')
root.title("LUDO GROUP 7")
...
dice_1 = PhotoImage(file="dice_face_1.gif")
...
Lcr = Label(root, text ="Credits", 
          width = 15, height = 2) 
Lcr.pack() 
Lcr.place(x=840,y=680)
m = Menu(root, tearoff = 0) 
m.add_command(label ="AYUSH SINGH") #18075015
m.add_command(label ="ASHT RASTOGI") #18075013
m.add_command(label ="ARYAN AGRAWAL") #18075011
m.add_command(label ="CSE DEPT., IIT BHU")
m.add_separator() 
m.add_command(label ="THANKS FOR PLAYING") 
...
Lcr.bind("<Button-1>", do_popup)

Lrules=Label(root, text ="Rules", 
          width = 5, height = 2)

Lrules.pack() 
Lrules.place(x=760,y=680)
menu = Menu(root, tearoff = 0) 
menu.add_command(label ="Unless you get a '6' when rolling the dice, your coins can't start")
menu.add_command(label ="Press the ROLL DICE button to roll the dice")
menu.add_command(label ="If it is a 6, then you can take another turn. So press ROLL DICE again") 
menu.add_command(label ="Click the coin which you want to move the first ROLL. The coin will move itself according to the number of the dice")
menu.add_command(label ="If you land on a colored square that coin can't be killed while at it")
menu.add_command(label ="To kill another color’s coin, you have to make your coin land on that box where other colored coin is")
menu.add_command(label ="If there are two coins of the same color on top of each other than that coin cannot be killed")
menu.add_command(label ="To win the game, your coins have to traverse around the board and come into the home lane")
menu.add_command(label ="If a player gets 3 six’s in a row, then his/her turn is void and next players plays")
menu.add_separator() 
menu.add_command(label =" ENJOY !!") 
...
Lrules.bind("<Button-1>", do_popu)

def board():                            #Drawing the board, piece by piece.

                                        #Splash Screen.
    tkinter.messagebox.showinfo(title=None, message="WELCOME ! \n TO START GAME CLICK OK & TO EXIT CLOSE THE GAME WINDOW")
    v = 0
    z = 0
    ...
    Label(image=logogs, width=42, height=42).place(x=(402), y=(52))
    ...
    Label(image=logoys, width=42, height=42).place(x=(652), y=(402))
    ...
    Label(image=logors, width=42, height=42).place(x=(52), y=(302))
    ...
    Label(image=logobs, width=42, height=42).place(x=(302), y=(652))
...
def turn():   #Prints whoose turn is it
    ...
    if RED == True:
        L2 = Label(root, text="   Red's Turn    ", fg='Red', background='green', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if BLUE == True:
        L2 = Label(root, text="   Blue's Turn   ", fg='Blue', background='green', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if GREEN == True:
        L2 = Label(root, text="Green's Turn  ", fg='light green', background='green', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if YELLOW == True:
        L2 = Label(root, text="Yellow's Turn", fg='yellow', background='green', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)
...
def roll():   #Rolling function that rolls a dice, goes again if its a six
    ...
    if TURN == True:
        ...
        if rollc == 1:
            ...
            if dice == 8:
                Label(image=dice_1, width=50, height=50).place(x=(800), y=(200))
            else:
                L1 = Label(root, text=dice, fg='WHITE', background='green', font=("Arial", 24, "bold"))
        ...
        if rollc == 2:
            if dice == 6:
                ...
                L3 = Label(root, text=dice1, fg='white', background='green', font=("Arial", 24, "bold"))
        ...
        if rollc == 3:
            if dice1 == 6:
                ...
                L4 = Label(root, text=dice2, fg='white', background='green', font=("Arial", 24, "bold"))
...
def kill(a,b,c,d,bh,ch,dh):   #function that determines if a gamepiece can be killed
    ...
    if ((a[bb].x == box[1].x and a[bb].y == box[1].y) or (a[bb].x == box[14].x and a[bb].y == box[14].y) or
        (a[bb].x == box[9].x and a[bb].y == box[9].y) or (a[bb].x == box[22].x and a[bb].y == box[22].y) or
        (a[bb].x == box[27].x and a[bb].y == box[27].y) or (a[bb].x == box[35].x and a[bb].y == box[35].y) or
        (a[bb].x == box[40].x and a[bb].y == box[40].y) or (a[bb].x == box[48].x and a[bb].y == box[48].y)):
        print("SAFE")
        #print(a[bb].x0)
...
button = Button(root, text="ROLL DICE", relief="raised", font=("Arial", 20),
                command=roll)  # call roll function everytime this button is clicked
